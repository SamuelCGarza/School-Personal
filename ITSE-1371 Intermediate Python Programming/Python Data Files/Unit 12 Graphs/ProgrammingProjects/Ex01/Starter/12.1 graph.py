...
class LinkedDirectedGraph(AbstractCollection):
    ...
    def addVertex(self, label):
        ...
        if label in self.vertices:
            raise AttributeError(f"Vertex with label {label} already exists in the graph.")
    ...
    def getVertex(self, label):
        """Returns the vertex with the given label if it exists in the graph."""
        if label not in self.vertices:
            raise AttributeError(f"Vertex with label {label} does not exist in the graph.")
    ...
    def addEdge(self, fromLabel, toLabel, weight):
        ...
        if fromLabel not in self.vertices or toLabel not in self.vertices:
            raise AttributeError("Vertices must already be in the graph.")
        
        fromVertex = self.getVertex(fromLabel)
        toVertex = self.getVertex(toLabel)
        
        if fromVertex.getEdgeTo(toVertex) is not None:
            raise AttributeError(f"Vertices {fromLabel} and {toLabel} are already connected by an edge.")
